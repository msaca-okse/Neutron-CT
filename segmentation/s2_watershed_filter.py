...
def place_medians_in_watersheds(image_XA, image_NA,eta_edge = 0.005,level = 0.15, conductivity = 0.5, smoothing_iter = 15, watershed_line = False, verbose=False):
    print('Watershedding with level ', level, ' and smoothing iter ', smoothing_iter)
    image_XA_ = sitk.GetImageFromArray(image_XA.astype(np.float32))
    image_NA_ = sitk.GetImageFromArray(image_NA.astype(np.float32))
    ...
    watershed_filter.SetMarkWatershedLine(watershed_line)#watershed_line)  # Prevent marking edges as lines
...
def watershed_filter(level, smoothing_iter):
    ...
    XA_pwc, NA_pwc = place_medians_in_watersheds(XA_surf_volume, NA_surf_volume, level = level, smoothing_iter = smoothing_iter)
    neutron = [[-0.01, 0.05],
            [-0.01,0.02], 
            [0.03, 0.1], 
            [0.02, 0.03], 
    ...
    segm = np.empty((len(neutron), nz, 10, nx), dtype=bool)
    ...
    segm_classes = np.empty((nz, 10, nx))
    for i in range(len(neutron)):
        segm_classes[segm[i]] = i+1

    T1 = time()
    Nz, Ny, Nx = np.shape(NA_surf_volume_)
    export_data = np.zeros((Nz, 10, Nx))
    export_data[subvol[0]:subvol[1],:,subvol[4]:subvol[5]] = segm_classes
    affine = np.eye(4)
    nii_img = nib.Nifti1Image(segm_classes.astype(np.uint8), affine)

    # Save the image to a file
    nib.save(nii_img, '/dtu-compute/msaca/cache/s1_wts_lvl_' + str(round(level,3)) +
         '_iter_' + str(smoothing_iter) + '.nii') 
    nii_img = nib.Nifti1Image(XA_surf_volume.astype(np.float32), affine)
...
if __name__ == "__main__":
    ...
    levels = np.linspace(0.05,0.15,6)
    smoothing_iters = np.array([30, 40, 50, 60, 70, 80], dtype=np.uint32)
    # Generate all combinations of level_XA and level_NA
    param_combinations = list(itertools.product(levels, smoothing_iters))

    # Create a multiprocessing pool
    with Pool() as pool:
        pool.starmap(watershed_filter, param_combinations)