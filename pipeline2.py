...
def pipeline(input_path,slices,output_path=None,angles = None,filter_type = None):
    ...
    if not cuda.is_available():
        ...
        sinograms.reorder('tigre')
        filter_type = 'ram-lak' if filter_type is None else filter_type
        fbp = FBP(sinograms, image_geometry=ig, filter=filter_type, backend='tigre')
        reconstruction = fbp.run()
